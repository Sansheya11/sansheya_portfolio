...
function ArticlePortfolioItemTitle({ itemWrapper }) {
    const date = itemWrapper.locales?.date || itemWrapper.date || ""

    return (
        <div className={`article-portfolio-item-title`}>
            <div className={`article-portfolio-item-title-row`}>
                <h5 className={`article-portfolio-item-title-main`}
                    dangerouslySetInnerHTML={{__html: itemWrapper.locales.title || itemWrapper.placeholder}}/>

                {date && (
                    <div className={`article-portfolio-item-date text-2`}>{date}</div>
                )}
            </div>
